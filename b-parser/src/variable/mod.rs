...
pub use crate::traits::{PureType, VariableConversion};
...
use keyword::attributes::{AttributeKeyword, UserDefinedTypes};
use keyword::functions::FunctionKeyword;
use modifiers::push::Push;
use name::VariableName;
use traits::{PureType, VariableConversion};
use utils::display;
use value::VariableValue;

use crate::keyword::control_flow::types::colon_ast::ColonAstCtrl;
...
use crate::tree::{Ast, CanPush, PushAttribute};
...
impl Push for Variable {
    fn push_block_as_leaf(&mut self, ast: Ast) -> Result<(), String> {
        #[cfg(feature = "debug")]
        crate::errors::Print::push_leaf(&ast, self, "var");
        if self.full {
    ...
    {
        #[cfg(feature = "debug")]
        crate::errors::Print::push_op(&op, self, "var");
        match &mut self.value {
