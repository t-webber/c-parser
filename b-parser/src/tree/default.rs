...
//! [`Push`](crate::modifiers::push::Push) for [`Ast`], but also to
//! simplify the api of [`Ast`].

use super::Ast;
use super::can_push::{AstPushContext, CanPush as _};
use crate::keyword::control_flow::node::ControlFlowNode;
use crate::keyword::control_flow::traits::ControlFlow as _;
use crate::literal::Attribute;
use crate::modifiers::push::Push as _;
use crate::operators::{Binary, Ternary, Unary};
use crate::symbols::{BracedBlock, Cast, ListInitialiser};
use crate::variable::{PureType as _, VariableConversion as _};
...
impl Ast {
    ...
    pub fn push_braced_block(&mut self, braced_block: Self) -> Result<(), String> {
        #[cfg(feature = "debug")]
        crate::errors::Print::push_leaf_in(&braced_block, "braced", self, "ast");
        match self {
