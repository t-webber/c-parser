...
impl Push for Ast {
    fn push_block_as_leaf(&mut self, ast: Self) -> Result<(), String> {
        #[cfg(feature = "debug")]
        crate::errors::Print::push_leaf(&ast, self, "ast");
        match self {
            Self::Empty => {
                *self = ast;
                Ok(())
    ...
    {
        #[cfg(feature = "debug")]
        crate::errors::Print::push_op(&op, self, "ast");
        match self {
...
impl PushAttribute for Ast {
    fn add_attribute_to_left_variable(
        ...
        crate::lgp!(
            ...
            crate::tree::repr_vec(&previous_attrs)
        );
...
///
/// If two consecutive literals are found, the [`crate`] fails, and this
