...
use utils::{IntoError as _, LocationPointer};
use crate::numbers::literal_to_number;
use crate::state::{LexingState, SymbolState};
use crate::types::{Ident, LexingData, Token};
...
pub fn end_symbols(
    ...
    for _ in 0u32..3u32 {
        ...
        if let Some((tok_len, symbol)) = symbols.try_to_operator(lex_data, location) {
             ...
             * because crate::symbols is not empty. */
        }
    }
}
