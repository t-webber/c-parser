...
use utils::{ErrorLocation, ExtendErrorBlock, IntoError as _, LocationPointer, display};
...
impl Token {
    ...
    pub const fn as_value_mut(&mut self) -> &mut TokenValue {
        &mut self.value
    }

    /// Converts a `char` into a token of value [`TokenValue::Char`]
    #[must_use]
    pub fn from_char(ch: char, location: &LocationPointer) -> Self {
    ...
    /// [`TokenValue::Number`].
    #[must_use]
    pub const fn from_number(number: Number, location: ErrorLocation) -> Self {
    ...
    /// [`TokenValue::Symbol`].
    #[must_use]
    pub fn from_symbol(symbol: Symbol, len: usize, location: &LocationPointer) -> Self {
    ...
    /// symbol.
    #[must_use]
    pub fn from_symbol_with_offset(
        symbol: Symbol,
        len: usize,
        offset: usize,
    ...
    /// Returns the value and the location of the [`Token`]
    #[must_use]
    pub fn into_value_location(self) -> (TokenValue, ErrorLocation) {
        (self.value, self.location)
    }
}
