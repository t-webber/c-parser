...
impl LocationPointer {
    ...
    #[coverage(off)]
    pub fn incr_col<F: FnMut(CompileError)>(&mut self, store: &mut F) {
    ...
    #[coverage(off)]
    pub fn incr_line<F: FnMut(CompileError)>(&mut self, store: &mut F) {
        self.col = 0;
    ...
    #[must_use]
    pub fn into_block(self, other: &Self) -> ErrorLocation {
    ...
    /// # Panics
    ///
    /// Panics if the offset is higher than the column number.
    #[must_use]
    pub fn to_past(&self, len: usize, offset: usize) -> ErrorLocation {
        ErrorLocation::Token(
            self.file.clone(),
            self.line,
